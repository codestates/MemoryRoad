...
import Navigation from '../../components/Navigation'; // 헤더
import TimeLineSideBar from '../../components/timeLineSideBar/timeLineSideBarForModify'; // 오른쪽 사이드바
...
function ModifyPinMap() {
  const { id } =
    useParams(); /* useParams로 routeId값 받아오기. */ /* useLayoutEffect 로 routeId값 넣어서 axios로 요청 받은 걸로 상태 업데이트하기. */

  const dispatch = useDispatch();

  /* 일단 테이스트는 목업데이터로. -> axios요청 받아오자마자 상태업데이트하기 --------------------useLayoutEffect */
  const [data, setData] = useState(true);
  const routeInfo = testData.route;
  const pinsInfo = routeInfo.Pins;
  if (data) {
    batch(() => {
      dispatch(setPinsInfoForModify(pinsInfo));
      dispatch(setPinsImageFilesForModify(pinsInfo));
      dispatch(setPinsPositionForModify(pinsInfo));
    });
    setData(false);
  }
  /* */
  const state = useSelector((state: RootState) => state.modifyRouteReducer);
  const {
    modifiedPins,
    modifiedFiles,
    modifiedPinPosition,
  ...
  const [isModalOpen, setIsModalOpen] = useState(false); // '저장' 모달창 오픈 여부
  ...
  console.log('modifiedPinPosition', modifiedPinPosition);
  console.log('modifiedPins', modifiedPins);
  // 아니 왜 여기서 이중for문을 돌면서 join을 하고 앉았냐고
  const pinCards = modifiedPinPosition?.map((card: any) => {
    // 맨 앞 null data 제거해줍니다.
    let newObj = card;
    modifiedPins.forEach((el: any) => {
      if (el['id'] === card.id) {
  ...
  if (saveBtnClick) {
    /* redux-persist test */
    const id = modifiedPinPosition?.length;
    const ranking = modifiedPinPosition?.length;
    const latlng: any = [
    ...
    batch(() => {
      dispatch(addPinInfo(id, ranking, pinTitle, currMarkerInfo)); // 첫번째 저장은 빨리 됩니다.
      dispatch(addPinImageFiles(id, ranking, arr));
      dispatch(addPinPosition(id, pinTitle, latlng));
    });
  ...
  useEffect(() => {
    ...
    /* 저장된 핀과 선을 지도에 표시 */
    // const linePath = [];
    if (modifiedMapPinPosition) {
      // 왜 자꾸 사라지니 ..
      for (let i = 0; i < modifiedMapPinPosition?.length; i++) {
        /* pinPosition 안에 latlng 요소가 있을 때 */
        if (modifiedMapPinPosition[i].latlng) {
          const lat: any = modifiedMapPinPosition[i]?.latlng[0];
          const lng: any = modifiedMapPinPosition[i]?.latlng[1];
          // linePath.push(new kakao.maps.LatLng(lat, lng)); // 성 생성
          if (i > 1) {
            const prevLat: any = modifiedMapPinPosition[i - 1]?.latlng[0];
            const prevLng: any = modifiedMapPinPosition[i - 1]?.latlng[1];
            const linePath = [
